...
**Note:** The Expressive theme is being integrated into Carbon Design System’s core library, and so expressive and productive styling will be available from one library source. After this change, the Expressive theme will no longer be maintained.
...
We are currently working on a new Sketch kit that will include pre-built versions of the Carbon for IBM.com components. We will post updates when the new kit is available.

In the meantime, follow the directions below to begin building out your designs using elements from the Carbon Expressive Sketch kit.

**1. Install Sketch**

To design with the Carbon Expressive kit you need the most recent version of [Sketch](https://www.sketch.com) installed.

**2. Set up Box Drive**

If you use Box Sync, you will need to uninstall it to use Box Drive. Follow the steps on [Uninstalling Box Sync](https://community.box.com/t5/Using-Box-Sync/Uninstalling-Box-Sync-Manually/ta-p/51105?advanced=false&collapse_discussion=true&filter=location&location=category:English&q=unintall%20box%20sync&search_type=thread) to uninstall Box Sync. Then follow the steps on [Installing and Updating Box Drive](https://community.box.com/t5/Getting-Started-with-Box-Drive/Installing-and-Updating-Box-Drive/ta-p/37450) to install Box Drive.

**3. Request access to the design kit**

Join the [#ibm-design-kit](https://cognitive-app.slack.com/archives/CDT9TSUAJ) Slack channel and request access to the design kit Box folder by posting your IBM email. You'll receive an email notification when you have been added to the Box folder.
...
To add the libraries, from Sketch navigate to `Preferences → Libraries → Add Library` and select `Box → IBM Design kit → Digital Design → Carbon for IBM.com → Carbon Expressive (White theme)`. Follow the same instructions for the other theme libraries.
...
**Note:** For product design, the [Carbon kit](https://www.carbondesignsystem.com/get-started/design/#get-the-kit) is available via Sketch Cloud. You do not need access to request access to Box.
...
As mentioned above, we are working on a new Sketch kit for all of our components and will post updates when it is available. 

## Get Carbon for IBM.com components

_For IBMers only_: We provide Sketch files of all the Carbon for IBM.com components, at the different breakpoints. These are available on Box.

**Layout components:** Layout components show some of the common layouts used in IBM’s digital space. They are carefully designed and coded to communicate prescribed content types and are expressions of IBM’s distinct brand ethos.

**UI components:** UI components are designed and built to support or enable small UI tasks and actions. They can be paired with other components to create layout components, templates, and patterns.

**Feature flag:** These components are built under the feature flag branch and are being tested before adding to the library permanently. Use with caution as the Carbon for IBM.com team may sunset these components at any time.

**Design only:** These components have a design but are not yet available in code. If your team would like to build any of these components, please reach out to the Carbon for IBM.com team.


<CardGroup>
  <MiniCard
    title="Layout components" 
    href="https://ibm.box.com/s/67mx3puesjjz8lnosnrma8d45as3g33j">
  ...
  <MiniCard
    title="UI components" 
    href="https://ibm.box.com/s/yqwhif915fwn9avz8gytvkc1p1038ofo">
  ...
  <MiniCard 
    title="Design only components" 
    href="https://ibm.box.com/s/ixj9x7d1uis2eubqag3485cov7h2bsm1">
  ...
  <MiniCard 
    title="Feature flag components" 
    href="https://ibm.box.com/s/t4r3s0zsuo7ahbj0coi5yfy8ynzlgax0">
...
Our Sketch resources are set up to help you design using key IBM assets. Learn more about using the [2x Grid](designing#2x-grid), [Symbols](designing#symbols), [Text styles](designing#text-styles), and [Layer styles](designing#layer-styles) in Sketch. 
...
You can find the grid template in the [IBM Design template file](https://www.sketch.com/s/3a3f3f2d-94d7-4c16-8e2e-88ba80a6382e) in Sketch. 
...
At the top of your screen, navigate to `File → New file from Template` and select the "IBM Design template" file in Sketch. You'll always be able to find the template here.
...
To insert a Carbon for IBM.com text style, go to the main menu and select `Insert → Text Styles → Carbon Expressive`.
...
The menus show the fixed type styles in the first panel. Fixed means that the type size does not change across different breakpoints. There is also an option to view the expressive type styles, and these style are fluid which means they adjust according to breakpoint. Choosing expressive allows you to further select for the breakpoint, and from there the type style you need. You choose the color in the final menu. 
...
Color in the IBM Design System is carefully considered, and we discourage custom layer styles. Use predetermined overrides when possible.

#### Library menu navigation 

- The first layer shows the theme selection, that is Carbon Expressive (White theme). 
- The second layer shows core Carbon Expressive color tokens for the selected item, for example `field-01`.
