...
1. In the menu, click the Figma dropdown then select `Libraries`. Alternatively, click on the `Asset` panel, and then the open book, or `Library` icon.
...
1. Select the frame to apply the template to.
...
3. Select a component and drag it onto the canvas. Alternatively, search the component by name.
...
2. In the `Design panel → Text`, click on `Style`.
...
1. Select the text to change.
2. In the `Design panel → Fill`, click on `Style`.
3. Select the text style to apply. Hover for additional usage guidance.
