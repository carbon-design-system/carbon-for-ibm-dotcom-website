...
Here are some of the elements that are commonly used and currently available. As we continue to build and offer additioanl options, we will share new designs on this page.
...
2. When there is a background image in the lead space, using left-aligned layout is recommended regardless of the primary content alignement.
...
There are different height options available to accomodate different use cases. These heights are based on the mini units defined in IBM Design Langauge <a href="https://www.ibm.com/design/language/2x-grid#the-mini-unit" target="_blank">here</a>.
