...
The grid should also be centered when the screen width is larger than max width to present content front and center. Left aligned grid creates a awkward white space on the right that draws unnecessary attention.
...
Begin designing by parsing through the content for hierarchies, and start with the most essential, overarching concepts on the top, and granular details below, and the related or supportive content at the bottom. Try to consider the mobile layout first because it forces us to be disciplined in sequencing content in a logical order.

Reference Layout page for more guidance on [Content hierarchy](/guidelines/layout#content-hierarchy).
...
The left hand navigation panel is similar to a table of content, which hides on smaller screens.
...
With the documentation model, the Dotcom shell breaks out of grid and is full width. This keeps the right side of the Masthead elements (Search and Account functions) anchored to edge of screen.
...
<Caption>IBM Docs prototyped using documentation model</Caption>
...
The effect of having a max width is not visible at smaller screens. But once the screen is wider than 99rem, presence of a max width will limit the amount of content shown, and create large empty spaces at the same time. Considering the goal is to display maximum content, seeing large empty spaces is a frustrating experience.
...
If you have to use Carbon for IBM.com &mdash; perhaps because your application is external facing, and integrated with marketing content &mdash; you may use the Dotcom shell as well. In this this case, Dotcom shell follows the grid and stretches to full width.
