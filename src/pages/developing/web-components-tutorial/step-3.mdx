...
For more background on understanding how `Content section`, `Content block`, `Content group`, and `Content item` are used checkout the info
...
Since `Content block` is inside the `Content section` we will wrap `Content section` around `Content block`:
...
Congrats!!! You have now successfully implemented the first customized `Content block`.

## Add Content block with Card in Card and Content group

The next `Content block` will follow the first steps of the first `Content block`.
...
Lastly, `Content group` has the last comment and following the design we need to add a
