...
Now that we have a nice setup with `Dotcom shell` and a few out of the box components such as `Lead space` and `Table of contents`, let's try to customize some of our more flexible components like `Content block`.
