...
**Note:** This builds on top of step 3, but be sure to check out the upstream step 4 branch because it includes the
...
<!-- prettier-ignore-start -->
```html path=src/index.html
      <a name="5" data-title="Carbon components"></a>
      <dds-content-section>
...
Notice the last selector, which will ensure the rules using our color tokens are used in all classes ending in `-theme`.
As our components are already using the `White` theme by default, we don't need to create a class for it.
