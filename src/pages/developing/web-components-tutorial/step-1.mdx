...
<Link to="/components/footer">footer</Link> to the page, there are more steps
necessary to properly configure those components in order to load in the proper
language and locale translation files, as well as the available translation
options in the footer's locale selector.
...
<Link to="/developing/building-for-ibm-dotcom#digital-data-object"> Building for
IBM.com</Link>, though to continue to properly set up the Dotcom Shell for this
tutorial, we will set the correct fields to properly set the language and
locale:
...
as the preferred URL strategy).

In this tutorial, we will use the query parameters of the url in order to inform
what the page's language/locale is. To do this, let's add the following in the
`<head>` below the `window.digitalData` declaration:
