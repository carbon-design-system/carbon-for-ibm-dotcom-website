---
title: Cards
description: Cards are workhorse components because they provide effective calls to action, and the various designs available work with a wide range of content.
---
...
![From top to bottom: Card, Card in card, Card link, and Feature card.](../../images/component/card/cards-preview-image-alt-5.png)
...
While the entire default Card is clickable, the Static modifier makes it so only the text link is clickable on the card. This is useful for when you have multiple interactive elements on a single card such as CTAs and Tag links.
...
Unlike Card, which can stand on its own, Card link should be presented with copy - it should never appear on its own. Card links contain a heading, an optional CTA (call-to-action), and optional body copy. The entire card is clickable.
...
4. **Body copy:** Optional body copy can be added for additional descriptions.
