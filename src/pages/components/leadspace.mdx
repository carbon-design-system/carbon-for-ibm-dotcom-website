...
1. **Background media:** Optional component that adds a background image or video with a gradient for accessibility.
...
All Lead space variants have the option to include a background image or video to create visual interest from the first moment in an experience. In both cases, a gradient is overlaid on top of the media so that the Lead space content passes accessibility requirements.
...
![Lead space with video](../../images/component/lead-space/component--lead-space--tall--background-image.jpg)

<Caption>Lead space tall with background image</Caption>
...
![Lead space with video](../../images/component/lead-space/component--lead-space--super--background-video.jpg)
...
The gradient can be added or removed based on the asset needs. We strongly recommend using the gradient when the heading content would be difficult to read due to low contrast with the background. If using an illustration, be aware of visual elements that would overlap with the lead space copy that would make it harder to read without a gradient. See the [design specs](https://www.figma.com/file/Gfb6xdHwKdBgNloNqlSbnG/Lead-space-v2?type=design&node-id=1031%3A20561&mode=design&t=xrvdzPlkLXOPoTg8-1) for illusrtation guidance by lead space size.
...
The lead space heading can be modified to highlight a word or phrase. This can be used to highlight product names, industries, or particular value. The highlighted word or phrase can be anywhere in the heading, but it should only ever appear once in a single lead space. This only applies to the heading, and does not extend to any other lead space element.

### Parent navigation

All Lead space variants have the option to include parent navigation of breadcrumbs or tag links to aid in way-finding.
...
<Caption>Lead space super with breadcrumbs</Caption>

#### Tag links

A collection of [Tag links](https://www.ibm.com/standards/carbon/components/tag-link) can be added to the Lead space, and always appear above the headline to establish context. Tag links can be clicked to navigate to other pages within IBM.com. Tag links work well in cases where the navigation does not have a clear, linear hierarchy. We recommend using more than one Tag link to represent a polyhierarchical structure.

![Lead space with tag links](../../images/component/lead-space/component--lead-space--medium--tag-links.jpg)

<Caption>Lead space medium with tag links</Caption>
