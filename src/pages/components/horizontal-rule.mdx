---
title: Horizontal rule
description: The Horizontal rule can be used as a graphical element to add structure, introduce breaks in the content, or delineate pieces of information.
---

<PageDescription>

The Horizontal rule can be used as a graphical element to add structure, introduce breaks in the content, or delineate pieces of information.

</PageDescription>

<AnchorLinks>

<AnchorLink>Default</AnchorLink>
<AnchorLink>Variations</AnchorLink>
<AnchorLink>Design and functional specifications</AnchorLink>
<AnchorLink>Development documentation</AnchorLink>
<AnchorLink>Feedback</AnchorLink>

</AnchorLinks>

## Default
Horizontal rules provide thematic breaks between content.

<Row>
<Column colMd={8} colLg={8}>

![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-default.jpg)

</Column>
</Row>

## Variations

### Fixed
In this variation, the Horizontal rule length sizes don't change based on browser size and there are several lengths or this variation usage.

<Row>
<Column colMd={8} colLg={8}>

![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-fixed.jpg)

</Column>
</Row>

<Title>Use case</Title>

![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-fixed-use-case.jpg)

### Fluid
In this variation, the Horizontal rule resizes as the window size is changed. The overhang and inset are the two main styles for this variation. 

<Row>
<Column colMd={8} colLg={8}>

![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-fluid.jpg)

</Column>
</Row>

<Title>Use case</Title>


![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-fluid-use-case-01.jpg)


![Image of Table of contents component](../../images/component/horizontal-rule/horizontal-rule-fluid-use-case-02.jpg)

## Design and functional specifications

The design specs and functional specs for Table of contents can be viewed <a href="https://ibm.box.com/s/ladjc15eyxg61c5wqbshggm3e4zgrnp5" target="_blank">here</a>.

## Development documentation

| Language      | Status        | Sandbox                                                                              |
| ------------- | -----------   | ------------------------------------------------------------------------             |
| React         | <div><span class="bx--tag bx--tag--green"><pre>Stable</pre></span></div>        | <a href="https://ibmdotcom-react.mybluemix.net/?path=/story/components-horizontalrule--default" target="_blank">View storybook</a>     |
| Web Components       | <div><span class="bx--tag bx--tag--cool-gray"><pre>Under construction</pre></span></div>  | Coming soon |

## Feedback

Help us improve this component by providing feedback, asking questions, and leaving any other comments on <a href="https://github.com/carbon-design-system/ibm-dotcom-library" target="_blank">GitHub</a>.
