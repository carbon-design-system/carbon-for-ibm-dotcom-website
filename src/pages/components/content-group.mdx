...
Keep in mind that the content group child container is positioned between the description and the CTA. As child content grows, or multiple children are added, the content group will push the CTA down to below the child content.

Avoid stacking CTAs at the end of a content group – this can overwhelm the user when a single CTA would be more effective. Keep this in mind as you add child components which might also have their own CTAs at the bottom.

Content group can contain multiple [content items](../components/content-item) as children. When using multiple content items, you should avoid mixing too much variation in the content items in a particular content group. For example, if using a content items featuring pictograms, make sure all the content items in this content group feature pictograms. This creates consistency for the end user. It is okay to have a content group with only one content item.
