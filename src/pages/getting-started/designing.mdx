---
title: Designing
description: Rapidly build beautiful and accessible experiences. The IBM.com Expressive and IBM.com Library kits contain all the resources you need to get started.
---

<PageDescription>

Rapidly build beautiful and accessible experiences. The IBM.com Expressive and IBM.com Library kits contain all the resources you need to get started.

</PageDescription>

## Introduction
The IBM.com Expressive and IBM.com Library kits are currently available for Sponsor Adopters only. The Carbon for IBM.com team is in the midst of development and has limited bandwidth to provide support to all IBM teams at this time. We plan to release the kits more broadly once the components are production ready. 
...
**1. Install Sketch.**
To design with the IBM.com Expressive kit you must have the most recent version of [Sketch](https://www.sketch.com) installed.
...
To add the libraries, from Sketch navigate to `Preferences → Libraries → Add Library` and select `Box →  IBM.com design kit → IBM.com Expressive (White theme)`.
Follow the same instructions for the other theme libraries and the IBM.com Library sketch library.

There are four IBM.com Expressive themes, two light (White and Gray 10) and two dark (Gray 90 and Gray 100). Each theme lives in its own Sketch library. You can subscribe to as many libraries as you’d like.
...
The full icon library and additional color collections live in the [IBM Design Language library](https://www.ibm.com/design/language/).

<Row className="resource-card-group">
  <Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="IBM Design language library"
      href="http://sketch//add-library/cloud/nwqmk"
    >
...
![Sketch](./../../images/icon/bee_icon.svg)
   ...
   <Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      ...
      href="http://sketch//add-library/cloud/KW2yr"
    >
...
## Start designing
To get started with the IBM.com Expressive and IBM.com Library kits, familiarize yourself with the contents of each library and template.
...
IBM.com Expressive components live in the design kit as Sketch symbols.
...
From the Insert menu, select `Symbols → IBM.com Expressive` to add symbols onto your page.

IBM.com Expressive symbols are built to be flexible, and designers should not detach symbols from the library. Once a symbol is detached, you will no longer receive updates as they are released.
...
Library menu navigation From the main menu select `Insert → Text Styles → IBM.com Expressive` to insert a IBM.com Expressive text style.

Text styles within components are carefully considered. We do not recommend detaching the symbol to change default styles.

#### Structure 

IBM.com Expressive text styles in Sketch are organized similarly to symbols.

Fixed sizes
* The first text style level shows the Carbon Expressive type token (i.e. body-long-01)
* The second level selects the type color, which is labeled with a Carbon Expressive color token name (i.e. text-01)

#### Expressive sizes

* The first text style level displays the expressive type styles
* The second level shows the breakpoints (i.e. lg (1056))
* The third level shows the Carbon Expressive type token (i.e. body-long-01)
* The fourth level selects the type color, which is labeled with a Carbon Expressive color token name (i.e. text-01)
...
– The first layer is the theme selection (i.e. IBM.com Expressive (White theme)). 
– The second layer is core IBM.com Expressive color tokens (i.e., field-01) for the selected them.
...
### Support
If you experience any issues while getting set up with IBM.com Expressive or IBM.com components kits, please head over to the [GitHub repo](https://github.com/carbon-design-system/carbon-for-ibm.com-design-kit) and please [create an issue](https://github.com/carbon-design-system/carbon-for-ibm.com-design-kit/issues/new) if your issue does not already exist.
